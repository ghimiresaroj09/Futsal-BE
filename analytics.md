...
The analytics API is exposed at `/api/v1/analytics/`. It returns all Analytics page data in one response.
...
```http
GET /api/v1/analytics/?period=6m&timezone=Asia%2FKathmandu
```
...
- Never calculate analytics from frontend dummy data after backend integration; replace the dummy values with `GET /api/v1/analytics/` response data.
